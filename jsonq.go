...
func (j *JSONQ) From(node string) *JSONQ {
	j.node = node
	v, err := getNestedValue(j.jsonContent, node, j.option.seperator)
	if err != nil {
		j.addError(err)
	}
	j.jsonContent = v
...
func (j *JSONQ) findInMap(vm map[string]interface{}) []interface{} {
	...
	for _, qList := range j.queries {
		...
		for _, q := range qList {
			...
			nv, errnv := getNestedValue(vm, q.key, j.option.seperator)
			if errnv != nil {
				j.addError(errnv)
				andPassed = false
			} else {
	...
	if aa, ok := j.jsonContent.([]interface{}); ok {
		for _, a := range aa {
			if vm, ok := a.(map[string]interface{}); ok {
				v, err := getNestedValue(vm, property, j.option.seperator)
				if err != nil {
					j.addError(err)
				} else {
...
func (j *JSONQ) distinct() *JSONQ {
	...
	if aa, ok := j.jsonContent.([]interface{}); ok {
		for _, a := range aa {
			if vm, ok := a.(map[string]interface{}); ok {
				v, err := getNestedValue(vm, j.distinctProperty, j.option.seperator)
				if err != nil {
					j.addError(err)
				} else {
...
func (j *JSONQ) sortBy(property string, asc bool) *JSONQ {
	...
	sm := &sortMap{}
	sm.seperator = j.option.seperator
	sm.key = property
	if !asc {
		sm.desc = true
	}
	...
	if aa, ok := j.jsonContent.([]interface{}); ok {
		for _, am := range aa {
			...
			for _, prop := range properties {
				...
				rv, errV := getNestedValue(am, node, j.option.seperator)
				if errV != nil {
					j.addError(errV)
					continue
				}
